    ...
    if pe_now is None:
        ...
        if pe_hist is not None and not pe_hist.empty:
            ...
            fig.add_trace(go.Scatter(
                ...
                fill="tozeroy",
                fillcolor="rgba(139,127,181,0.08)",
            ))

            if stats["mean"]:
                fig.add_hline(
                    y=stats["mean"], line_dash="dash", line_color="#facc15",
                    annotation_text=f"TB {stats['mean']:.1f}x",
                    annotation_position="top right",
                )
            if stats["mean"] and stats["stdev"]:
                fig.add_hline(
                    y=stats["mean"] + stats["stdev"], line_dash="dot",
            ...
            # Đường P/E hiện tại
            fig.add_hline(
                y=pe_now, line_color="#f97316", line_width=2,
        ...
        # Bảng thống kê chi tiết (collapsible)
...
def _filter_and_clean(results_df, signal_filter):
    """Áp bộ lọc Trạng thái + loại cột rác. Dùng chung cho cả 2 tab."""
...
def render_screener_results(results_df, signal_filter):
    """Tab 📊 Kết Quả Quét: Tổng Quan + 3 Đường Định Giá."""
