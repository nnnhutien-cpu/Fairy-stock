...
def _filter_and_clean(results_df, signal_filter):
    """Áp bộ lọc Trạng thái + loại cột rác. Dùng chung cho cả 2 tab (Kết Quả Quét & Tín Hiệu)."""
...
def render_screener_results(results_df, signal_filter):
    """Tab 📊 Kết Quả Quét: Tổng Quan + 3 Đường Định Giá (KHÔNG gồm Cảnh Báo & Tín Hiệu,
    phần đó đã tách riêng sang render_screener_signals / tab 📡 Tín Hiệu & Cảnh Báo)."""
...
def render_screener_signals(results_df, signal_filter):
    """Tab 📡 Tín Hiệu & Cảnh Báo: RSI/MFI, Tín Hiệu Sideway, Cảnh Báo Tạo Đỉnh, Bắt Đáy.
    Tách riêng khỏi Kết Quả Quét vì bảng đó đã đủ rộng, để hai bên đỡ phải cuộn ngang."""
    df = _filter_and_clean(results_df, signal_filter)
    if df.empty:
