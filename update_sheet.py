...
import os
import json
from google.oauth2.service_account import Credentials # DÙNG THƯ VIỆN CHUẨN, BỎ OAUTH2CLIENT
...
def main():
    ...
    # Quét 50 mã để test nhanh (Sau này chạy mượt bạn xóa chữ [:50] đi)
    for ma in danh_sach_ma[:50]:
        try:
            ...
            if df_hist is not None and not df_hist.empty and len(df_hist) >= 20:
                ...
                # CHỈ LẤY ĐÚNG 1 DÒNG CỦA NGÀY HÔM NAY
