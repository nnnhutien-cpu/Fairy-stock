...
# "đường cùng nằm TRÊN MÂY" (bảng kỹ thuật) — hai câu chỉ nhất quán nếu
# "mây" được hiểu là dải nằm giữa Knife1(65) và Knife2(129) (đóng vai trò
# Senkou A/B của hệ mới).
...
def calculate_technical_signals(
    ...
    k65=65, k129=129,                                     # 2 đường định giá "Cô Tiên"
    ...
    # ------------------------------------------------------------------
    # 5. HAI ĐƯỜNG ĐỊNH GIÁ "CÔ TIÊN" — KHÔNG dịch, công thức (HHV+LLV)/2
    # ------------------------------------------------------------------
    ...
    # Hướng đi (so với chính nó N phiên trước) — dùng 5 phiên để lọc nhiễu
    ...
    # ------------------------------------------------------------------
    # 9. XU HƯỚNG THEO 2 ĐƯỜNG "CÔ TIÊN" (BẮT ĐẦU tăng/giảm — snapshot)
    # Quan trọng nhất: Knife1(65) & Knife2(129) tạo mây nội bộ dốc lên/
    # xuống và cùng đi lên/xuống; giá đóng cửa xác nhận nằm trên/dưới mây.
    # ------------------------------------------------------------------
    ...
    all2_up   = knife_core_up   and close > latest['fmay_top']
    all2_down = knife_core_down and close < latest['fmay_bot']
    ...
    if all2_up and not end_uptrend:
        xu_huong = "🟢 Tăng"
    elif all2_down and not end_downtrend:
        xu_huong = "🔴 Giảm"
    else:
        xu_huong = "🟡 Sideway"
